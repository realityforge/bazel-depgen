...
This workflow publishes bazel-depgen to Maven Central using the split release scripts in this directory. Run commands
...
* Maven Central username is the fixed legacy user `realityforge`.
...
The package command writes `dist/bazel-depgen-<version>/` and `dist/bazel-depgen-<version>.zip`.
