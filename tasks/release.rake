...
### [v#{ENV['PRODUCT_VERSION']}](https://github.com/realityforge/bazel-depgen/tree/v#{ENV['PRODUCT_VERSION']}) (#{ENV['RELEASE_DATE']}) · [Full Changelog](https://github.com/realityforge/bazel-depgen/compare/#{from}...v#{ENV['PRODUCT_VERSION']})
HEADER
...
HEADER
    ...
    stage('GithubRelease', 'Create a Release on GitHub') do
      ...
      client.login
      client.create_release('realityforge/bazel-depgen', tag, :name => tag, :body => changes, :draft => false, :prerelease => prerelease)

      candidates = client.list_milestones('realityforge/bazel-depgen').select {|m| m[:title].to_s == tag}
      unless candidates.empty?
        milestone = candidates[0]
        unless milestone[:state] == 'closed'
          client.update_milestone('realityforge/bazel-depgen', milestone[:number], :state => 'closed')
        end
      end
    end
  end
