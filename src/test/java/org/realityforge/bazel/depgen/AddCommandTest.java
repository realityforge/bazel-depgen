...
{
  @Test
  public void add_noArtifactsSection()
    throws Exception
  {
    ...
    assertOutputContains( handler.toString(),
                          "Added dependency 'com.example:myapp:jar:1.0' to configuration file " +
                          environment.getConfigFile() );
    assertNoTempFiles( environment.getConfigFile().getParent() );
  }

  @Test
  public void add_ignoresCommentedTemplateArtifactsSection()
    throws Exception
  {
  ...
  @Test
  public void add_existingArtifactsSection()
    throws Exception
  {
    writeConfigFile( "artifacts:\n" +
    ...
    throws Exception
  {
    ...
    final AddCommand command = new AddCommand();
    final Environment environment = newEnvironment();
    assertTrue( command.processOptions( environment,
                                        "org.example:lib:1.2",
                                        "--nature",
                                        "java",
                                        "--nature",
  ...
  @Test
  public void add_repositoryNameStrategy()
    throws Exception
  {
    writeConfigFile( "options:\n" +
                     "  verifyConfigSha256: false\n" );
  ...
  @Test
  public void add_noIncludeSource()
    throws Exception
  {
    writeConfigFile( "options:\n" +
                     "  verifyConfigSha256: false\n" );
  ...
  @Test
  public void add_j2clNatureRequiresIncludeSource()
    throws Exception
  {
    ...
    assertEquals( exception.getMessage(), "Dependencies with the J2cl nature require includeSource to resolve to true." );
    assertEquals( loadAsString( environment.getConfigFile() ), original );
  }

  @Test
  public void add_conflictingIncludeSourceOptions()
    throws Exception
  {
    final TestHandler handler = new TestHandler();
    final AddCommand command = new AddCommand();
    assertFalse( command.processOptions( newEnvironment( handler ),
                                         ...
                                         "--include-source",
                                         "--no-include-source" ) );
    assertEquals( handler.toString(), "Error: Only one of --include-source or --no-include-source may be specified." );
  }

  @Test
  public void add_j2clSuppressRequiresLibraryMode()
    throws Exception
  {
    final String original =
      "options:\n" +
  ...
  public void add_duplicate()
    throws Exception
  {
    for ( final String existingCoord : Arrays.asList( "com.example:myapp",
  ...
  @Test
  public void add_natureSpecificOptionWithoutMatchingNature()
    throws Exception
  {
    final String original =
      "options:\n" +
    ...
    final DepgenValidationException exception =
      expectThrows( DepgenValidationException.class, () -> command.run( new CommandContextImpl( environment ) ) );
    assertEquals( exception.getMessage(), "The J2CL-specific options require the dependency to have the J2cl nature." );
    assertEquals( loadAsString( environment.getConfigFile() ), original );
  }

  @Test
  public void add_unsupportedInlineArtifactsSection()
    throws Exception
  {
    final String original =
      "artifacts: []\n";
  ...
  @Test
  public void add_candidateValidationFailureLeavesFileUnchangedAndCleansTempFile()
    throws Exception
  {
    final String original =
      "artifacts:\n" +
    ...
    assertEquals( exception.getMessage(),
                  "Artifact 'com.example:myapp' declared a repository named 'missing' but no such repository is declared in the repository section. Known repositories include: central" );
    assertEquals( loadAsString( environment.getConfigFile() ), original );
    assertNoTempFiles( environment.getConfigFile().getParent() );
  }

  @Test
  public void add_missingCoord()
    throws Exception
  {
    final TestHandler handler = new TestHandler();
    final AddCommand command = new AddCommand();
  ...
  private void assertNoTempFiles( @Nonnull final Path configDirectory )
    throws Exception
  {
