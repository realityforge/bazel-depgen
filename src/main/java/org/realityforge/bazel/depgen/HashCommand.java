        ...
        if (null == _expectedSha256) {
            ...
            if (logger.isLoggable(Level.WARNING)) {
                ...
                if (options.verifyConfigSha256()) {
                    final Path configPackage = options.getWorkspaceDirectory()
                            .relativize(model.getConfigLocation().getParent());
                    final Path configLocation =
                            configPackage.resolve(model.getConfigLocation().getFileName());
                    logger.log(
                            Level.WARNING,
                            "Depgen generated outputs for configuration file '" + configLocation
                                    + "' are out of date.");
                    logger.log(
                            Level.WARNING,
